...
<body>
	<div class="pagingArea" align="center">
		<!-- 맨 앞으로 이동 버튼 -->
	    <button class="btn btn-secondary btn-sm" id="startPage"><<</button>
		...
		<c:if test="${ requestScope.selectCriteria.pageNo > 1 }">
			<button class="btn btn-secondary btn-sm" id="prevPage"><</button>
		</c:if>
		...
		<c:forEach var="p" begin="${ requestScope.selectCriteria.startPage }" end="${ requestScope.selectCriteria.endPage }" step="1">
			<c:if test="${ requestScope.selectCriteria.pageNo eq p }">
				<button class="btn btn-secondary btn-sm" disabled><c:out value="${ p }"/></button>
			</c:if>
			<c:if test="${ requestScope.selectCriteria.pageNo ne p }">
				<button class="btn btn-secondary btn-sm" onclick="pageButtonAction(this.innerText);"><c:out value="${ p }"/></button>
			</c:if>
		</c:forEach>
		...
		<c:if test="${ requestScope.selectCriteria.pageNo < requestScope.selectCriteria.maxPage }">
			<button class="btn btn-secondary btn-sm" id="nextPage">></button>
		</c:if>
		
		<!-- 마지막 페이지로 이동 버튼 -->
		<button class="btn btn-secondary btn-sm" id="maxPage">>></button> 
	</div>
