...
CREATE TABLE HOTEL (
   ...
   HOTEL_INTRODUCE   VARCHAR2(1000) NOT NULL,
   LATITUDE    NUMBER,
   LONGITUDE 	NUMBER,
...
COMMENT ON COLUMN HOTEL.HOTEL_INTRODUCE IS '호텔 소개';
...
COMMENT ON COLUMN ROOM.ROOM_PERSON IS '정원';
...
CREATE TABLE RESERVE (
   ...
   RESERVE_PHONE VARCHAR2(50) NOT NULL,
...
COMMENT ON COLUMN RESERVE.RESERVE_NAME IS '예약자명';
COMMENT ON COLUMN RESERVE.RESERVE_PHONE IS '예약자전화번호';
COMMENT ON COLUMN RESERVE.PERSON_NUM IS '숙박인원';
...
COMMENT ON COLUMN AHOREPORT.REPORT_REASON IS '신고사유';
COMMENT ON COLUMN AHOREPORT.REPORT_CHECK IS '관리자확인사항';
...
CREATE TABLE QUESTION_PHOTO (
   ...
   CONSTRAINT FK_QUESTION_CODE FOREIGN KEY(QUESTION_CODE) REFERENCES QUESTION(QUESTION_CODE)ON DELETE CASCADE
);
